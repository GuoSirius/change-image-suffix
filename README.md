...
- ✂️ 空白裁剪：裁掉图片四周透明 / 纯白 / 自定义背景区域，输出到 `cropped/`
- 🖱️ Windows 右键菜单集成
...
注册后，在文件夹或图片文件上**右键 → 悬停**即可看到格式/裁剪子菜单：

```
🖼 转换图片 (cis)    ← 悬停展开
  ├── 📐 裁剪空白    ← 裁掉四周空白到 cropped/
  ├── 🌀 WebP       ← 默认
  ├── 📷 JPG
  ├── 🖼 PNG
  ├── 📺 AVIF
...
**适用场景：**
- 📁 文件夹空白处右键 → 裁剪 / 转换该目录所有图片
- 📁 文件夹图标上右键 → 裁剪 / 转换该目录所有图片
- 🖼 图片文件上右键 → 裁剪 / 转换该单个图片
- 🔢 支持多选文件/文件夹后右键
...
| `-f, --file <file>` | 指定单个文件转换（裁剪/转换通用） | - |
| `-h, --help` | 显示帮助 | - |
| `-v, --version` | 显示版本 | - |
| `crop [路径]` | 空白裁剪子命令，输出到 `cropped/`（可与 `-f`/`-p`/`-r`/位置参数组合） | - |
| `-b, --bg <r,g,b>` | 裁剪接近该 RGB 的背景色（默认仅裁透明 + 纯白） | - |
| `--padding <n>` | 裁剪后四周保留的留白像素 | 0 |
| `--tolerance <n>` | 背景色容差（每个通道差值上限） | 10 |
| `--white-tolerance <n>` | 白底容差（rgb 低于 255 的上限） | 8 |
...
```bash
# 裁剪单张图片，输出到 cropped/ 子目录
cis crop -f ./big.png

# 递归裁剪目录下所有图片
cis crop -p ./images -r

# 裁剪并保留 10px 边距（避免内容贴边）
cis crop -f ./big.png --padding 10

# 自定义背景色：把接近浅灰(245,245,245)的区域也裁掉，容差 5
cis crop -f ./big.png --bg 245,245,245 --tolerance 5
```

> ✂️ **裁剪规则**：默认裁掉透明像素 + 纯白背景；可用 `--bg` 指定任意背景色，`--padding` 控制四周留白，`--white-tolerance` / `--tolerance` 控制容差。输出文件名保持不变，放在 `<原目录>/cropped/`。
